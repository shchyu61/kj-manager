...
#   ・FIREBASE_CRED_ENV → 未設環境變數時，自動改讀同資料夾的憑證檔（沒有憑證時去重會退回本機紀錄檔）
#   ・TEST_MODE　　　　 → 見下方【TEST_MODE 設定指南】
# ══════════════════════════════════════════════════════════════
# 版本：見 SCRIPT_VERSION（全檔唯一版本識別處）
# 適用：台股全市場 + 美股清單 + 虛擬幣 + 外匯 + 黃金 + 三合一追蹤【安聯月配息基金】
# 通知方式：Gmail 發信到 shchyu61@gmail.com
# 三條路線（詳見首頁【條件ABCDEFW策略說明】）：
...
# ══════════════════════════════════════════════════════════════
# ★★★【TEST_MODE 設定指南】(09181047)　主帥 09/18 09:07、09:16 指示寫在此處
#     你AI應該要把建議，寫在檔案或程式碼旁邊才對吧。」
# ──────────────────────────────────────────────────────────────
# 【五種模式在做什麼】
#   False      ＝ 全市場掃描：台股、美股、虛擬幣、基金（月K OR 週K → 日K → 5分K OR 15分K，不含條件D）
#   True       ＝ 只寄兩封測試 Gmail 後結束，確認信箱與過濾器用，平常不要用
#   'futures'  ＝ 期貨掃描：台指 5分K ＋ 15分K 三道；週二～週五 09:05~13:30 發出的訊號會附【週選擇權履約價建議】
#   'condW'    ＝ 條件W 週選擇權：只跑第三道（5分K OR 15分K 的 V轉買 CALL／A轉買 PUT），
#                 時窗＝週二15:05~週三11:30、週四15:05~週五11:30
#   'intraday' ＝ 主帥專用即時路線：持股＋做空＋觀察清單的台股、美股、虛擬幣、外匯、黃金（不含期貨、不含基金）
# ──────────────────────────────────────────────────────────────
# 【主帥的本機兩支檔要怎麼設】★這是最常被問的一題
#   ・3_stock_monitor.py    → TEST_MODE = False      （全市場掃描，含基金）
#   ・4_futures_monitor.py  → TEST_MODE = 'futures'  （期貨＋週選擇權建議）
#   ★★★4_futures_monitor.py 【嚴禁改成 'condW'】（已澄清爭點 Ｃ-06）：
#     ・'futures' 有三道關卡，且 09:05~13:30 會附週選擇權履約價建議；
#     ・'condW' 只跑第三道、沒有前兩道保護，兩者【各缺一半】，不是誰取代誰。
#   ★兩支檔【除了這一行以外完全相同】（ＡＭ２９ 四版一致）：改了 3_stock_monitor.py 就要同步複製一份改名覆蓋。
# ──────────────────────────────────────────────────────────────
# 【雲端不用設】雲端由 .github/workflows 的 yml 以環境變數帶入：
#   stock_scan.yml（全市場＋期貨＋持股健檢）／condw_scan.yml（條件W）／intraday_scan.yml（主帥專用即時路線）
# ══════════════════════════════════════════════════════════════
# 驗證篩選模式（★詳細設定指南見本檔上方【TEST_MODE 設定指南】）：
#   False      = 全市場掃描（正式模式）
#   True       = 測試模式（只發測試 Gmail）
#   'futures'  = 期貨掃描（5分K ＋ 15分K 併行，含週選擇權履約價建議）
#   'condW'    = 條件W 週選擇權（只跑第三道，5分K OR 15分K）
#   'intraday' = 主帥專用即時路線（長期／中期＋短線兩條並行）
#   （沿革見檔尾【附錄．程式註解沿革存查】H-001）
TEST_MODE = False   # 切換：False / True / 'futures' / 'condW' / 'intraday'（✅09172001 主帥專用即時路線）
FUTURES_PERIODS = ['5m', '15m']   # ✅09140742 期貨掃描的週期清單（★現況即二者併行）

# ── 【期貨掃描專屬設定】（TEST_MODE = 'futures' 時啟用）──────────
# 執行時段（★唯一定義在 _condw_current_window()）：
#   ・日盤：週一~週五 09:05~13:30（另含台股盤中極端異動監控）
#   ・條件W 時窗：週二15:05 → 週三11:30、週四15:05 → 週五11:30
# 掃描標的：台灣加權指數（^TWII）作為台指期替代標的；夜盤無K棒，見第九章 9-9
...
# └──────────────────────────────────────────────────────────────────────
# ── 【條件W：週選擇權【雙向】專屬設定】（TEST_MODE = 'condW' 時才啟用）──
#   ✅09181106 標題更正：★現行為多空雙向（V轉買 CALL／A轉買 PUT，CONDW_ENABLE_SHORT=True），
#     ★★原標題「做多專屬」是 R-08（條件W 只做 buy call）時期的寫法，R-08 已撤銷。
...
#     「★★兩個方向的 K 棒根數一律 ★N ＝ 5（★不是近5根同時滿足碰軌與轉折的 3）」
...
# ✅09030300【★★★方向更正：AI 上一輪把 N 的效果講反了】
#   ★★AI 於 09022155 寫「N 越小，越容易滿足『近N根任一觸及布林軌』→ 訊號變多」
#     → ★★★這【完全寫反】。★正確是：
#     ・條件A（近N根【任一】最低價 ≤ 布林下軌）：★N 越大 → 涵蓋越多根 →
#       「任一」越容易成立 → ★★訊號【越多】。
#     ・條件B（近N根最低價【均】< 布林中軌）：★N 越大 → 「均」越難成立 →
#       ★★訊號【越少】。★★★兩者方向相反，★但 A 是主力路徑（最直接），
#       故整體淨效果由 A 主導：★N 越大 → 訊號越多。
#   ★★AI 還把「54／18」寫成看起來像除法的形式，★主帥讀成 54÷18＝3。
#     ★★★現行：★5分K 與 15分K【都用近 5 根】（碰軌丙案，主帥 09/17 08:46）；舊值 54／18 見檔尾附錄。
#
#   ★★★主帥 2026/09/03 02:02 裁示（原話）：
#     「我要的是【★訊號較敏感、觸發較頻繁，★★符合定稿『寧可多報，不可放過一個』】。
#       至於這個是甲還是乙，我看不懂？由AI依照這個原則決定。」
#   → ★碰軌丙案後不再等比換算：★★兩個週期一律近 5 根。
#   ★★★與交接文件第六章②「N＝5」的關係：★該定稿的脈絡是「事不過五 vs 近5根同時滿足碰軌與轉折」
#     （原文括號寫「不是近5根同時滿足碰軌與轉折的 3」），★★是在 3 與 5 之間選 5；
...
#   凍結清單 F-21 同步改為 5（原 54／18）。與主帥最早定稿「N＝5（不是近5根同時滿足碰軌與轉折的 3）」一致。
# ✅09170937【54／18 與 5 是什麼．主帥 09/17 09:37 要求加註，避免日後忘記】
#   ・「近N根」＝回頭看幾根K棒，用在條件W 的條件A（近N根曾碰下軌＋V轉）與條件B（近N根都在中軌下＋MACD柱連縮後放大）。
#   ・（舊值 54／18 的由來與失效原因見檔尾【附錄．程式註解沿革存查】）
#     現行兩個週期皆 CONDW_LOOKBACK＝5。
#   ・現值 5／5：5分K 看25分鐘、15分K 看75分鐘；碰軌後很快V轉才進場，條件B 可以運作；與全系統「近N根一律5」一致。
#   ・★與「台指期K棒累積器 54 根」不同：那是【資料量門檻】（累積滿 54 根才開始算指標），不是策略根數，★不可改成 5。
...
# ── 【２-7】做空入場策略：空頭三道關卡條件 ────────────────────────
# ⚠️ 做空策略嚴禁用於當沖或隔日沖（★此限制針對全市場掃描與家人親友房客；★★主帥專用短線路線另由 STOCK_SHORT_ROUTE_ENABLED 控管）
...
SHORT_LOOKBACK_BARS   = 3       # ✅09181047 全檔僅定義、無任何使用（歷史常數）；現行近N根一律 5（TOUCH_LOOKBACK_BARS）
...
cache_long_1mo = {} # ✅09150004 改名：長期投資【月K】快取（原名 weekly_cache 名實不符）
_rt_price_map = {}  # ✅08061155 即時價對照表 {ticker: 現價}，由 prefetch_realtime_prices() 批次填入
cache_mid_1wk_1d  = {} # ✅09150004 改名：中期投資【週K】＋日K 混用快取（cache_key 含 interval，不會互相覆蓋）
...
def get_active_markets():
    ...
    # ✅09151754【主帥 2026/09/15 07:15 核准甲＋乙】★★★虛擬幣脫離美股時段
    #     ★★一天只看 6.5 小時，★★★而虛擬幣是【24小時交易】。
    #   ★★【雙重死因】該時段內 cron 只有 22:35 一個點，
    #     ★而 22:35 又整段落在睡眠時段(21:30~07:30) → ★★缺陷F 吞掉訊號。
    #   ★★★【實例】主帥 09/15 06:54：黃立成比特幣 V 轉大賺，
    #     ★而主帥近2個月【從未收到 BTC/ETH/DOGE 任何進場通知信】。
    #   ★【通知量不會暴增】主帥 09/15 07:15 原話：「★★我所有的投資種類的
    #     進出場策略，★都有守門員和第一道和第二道的先決條件，
    #     ★★★怎麼可能會在第三道動不動就增加通知量？」
    #   ★★【外匯同時受惠】外匯的判定依賴 'CRYPTO' in active_markets
    #     （第5180行），★故本改動亦讓外匯恢復全時段。
    #   （沿革見檔尾【附錄．程式註解沿革存查】H-007）
    active.append('CRYPTO')  # ★24小時交易 → ★★全時段掃描，★★★不受美股時段限制
...
def print_feature_report():
    ...
    try:
        ...
        _mode = {'5mk': '期貨掃描(5分K+15分K)', 'futures': '期貨掃描(5分K+15分K)',
                 'condW': '條件W週選擇權（condw-scan job）',
                 'intraday': '主帥專用即時路線（intraday-scan job）'}.get(   # ✅09181118 補 intraday，原顯示成全市場掃描
        ...
        _rows = [
            ...
            ('觀察清單讀取（F-14）',      'watchlist',
             '本 job 不負責（只在全市場掃描執行）' if TEST_MODE in ('5mk', 'futures', 'condW')
             else '未執行到'),
            ('觀察清單補掃（F-14）',      'watchlist_extra', '未執行到'),
            ('台指期K棒累積（F-11）',     'txf_bars',
             '本 job 不負責（請看 futures-scan job）' if TEST_MODE not in ('5mk', 'futures', 'condW')
             else '未執行到（非期貨時段）'),
...
def _one_txf_sample():
    ...
    try:
        ...
        #   （沿革見檔尾【附錄．程式註解沿革存查】H-009）
        # ✅09240542【CTime 去重】主帥 09/24 05:42「第 1 件：做」：期交所報價時間（CTime）與上一筆相同＝報價未更新，
        #   ★不寫入，避免同一筆報價被存成價格不變的「死棒」（台指期K棒累積器品質驗證報告 08230324 不通過原因之一）
        _ct = str(_snap.get('time') or '') if isinstance(_snap, dict) else ''
        if _ct and _bars and _bars[-1].get('ct') == _ct:
            print(f"  \U0001F501 台指期快照 CTime={_ct} 與上一筆相同（報價未更新）→ 不寫入，避免死棒")
            return True
        ...
        _bars[-1]['dl1'] = _dl
        _bars[-1]['dv1'] = _dv
        _bars[-1]['ct'] = _ct   # ✅09240542 記下本根最後一筆報價時間，供下次去重
...
def check_tw_intraday_extreme():
    ...
    try:
        ...
        _body = '\n'.join([
            ...
            '',
            '★這是【盤中即時】通知，行情仍在進行中。',
            '　（收盤確認信已停用：TW_CLOSE_CONFIRM_ENABLED=False）（主旨不含「盤中即時」）。',
        ])
...
def check_buy_precondition(df, is_weekly=False):
    try:
        ...
        #   （沿革見檔尾【附錄．程式註解沿革存查】H-013）
        cond_D = False

        _abc = cond_A or cond_B or cond_C
        # ✅09141319【主帥 09/14 08:21 明示】★「條件D，★★只會發生在第三道，
        #   ★★★絕對不可發生在【第一道 or 第二道】！」
        #   ★★改為：★第一道只認 A/B/C；★★condD 旗標仍回傳，
        #   ★★★供上層第三道（result['bull_d']）獨立走條件D 路徑使用。
        #   （沿革見檔尾【附錄．程式註解沿革存查】H-014）
        return _abc, cond_D  # (第一道是否通過[只認ABC], 是否由條件D觸發)
    except Exception as e:
...
#   ・觸發前提由【第一道 AND 第二道】滿足（主帥 09/18 02:05、02:34），★嚴禁把「曾碰下軌」寫進本函式（已澄清爭點 Ｃ-01）。
# ============================================================
...
def check_short_precondition(df, is_weekly=False):
    ...
    try:
        ...
        return _abc_s, cond_D_short  # (第一道是否通過[只認ABC], 是否由條件D空頭觸發)
    except:
...
def analyse_market_index(ticker, label):
    ...
    try:
        ...
        print(f"  {label}大盤判定：{' / '.join(flags)}（月K RSI:{result['rsi_w']:.1f} 日K RSI:{result['rsi_d']:.1f}）")
    except Exception as e:
...
def _full_third_gate(df, is_long):
    """✅09191108【全市場掃描第三道．改用 stage_pass】主帥 09/18 00:00：5分K OR 15分K、不含條件D；
    ★09/19 09:43「乙的加強版」後，判斷內容與主帥專用路線【完全相同】：
...
def scan_stock(ticker, is_holding=False, _mode_label=None):
    ...
    global cache_long_1mo, cache_mid_1wk_1d
    ...
    try:
        ...
        df_w = get_stock_data(ticker, period='5y', interval='1mo', cache=cache_long_1mo)  # ✅ v05170856 長期投資改月K
        ...
        if is_holding:
            ...
            _df_wk  = get_stock_data(ticker, period='2y', interval='1wk', cache=cache_mid_1wk_1d)
        ...
        if not TEST_MODE:
            if SCAN_MODE == 'daily':
                ...
                _df1st = get_stock_data(ticker, period='2y', interval='1wk', cache=cache_mid_1wk_1d)  # ✅ v05170856 中期投資改週K
        ...
        df_d = get_stock_data(ticker, period='2y', interval='1wk', cache=cache_mid_1wk_1d)  # 週K（保留供參考）
        ...
        _df_1d = get_stock_data(ticker, period='1y', interval='1d', cache=cache_mid_1wk_1d)
        ...
        # ── 第三道：5分K OR 15分K（✅09180937 主帥 09/18 00:00 定案）────────
        #   ★★主帥 09/18 00:00：「我怎麼可能同意只有5分K，而讓家人親友房客沒有15分K判斷是否A轉或V轉可以逃命呢！」
        #   ★★★規則與主帥專用即時路線第三道相同，唯一差別＝不含條件D（不讓家人親友房客追高）。
        #   （沿革見檔尾【附錄．程式註解沿革存查】H-015）
        _now_ts15 = time.time(); _ck15 = f"15m_{ticker}"
        if _ck15 in five_min_cache and (_now_ts15 - five_min_cache[_ck15]['ts'] < 600):
        ...
        _g3_short = bool(_full_third_gate(df_5m, False) or _full_third_gate(df_15m, False))
        _g3_label = ('5分K' if _full_third_gate(df_5m, _is_long_ok) else '15分K')

        if _is_long_ok:
            # ── 多頭第三道：5分K OR 15分K（條件A/B/C OR E OR eLeader）AND 前1根→當根 V轉 AND RSI 門檻
            if not _g3_long:
                return None
...
def check_overnight_extreme_move():
    ...
    EWT 漲跌達 TW_EXTREME_PTS（現值 750 點，約 1.67%）→ 立即發 Gmail 警報（★漲跌【兩個方向】都發，每方向每日最多 2 次）
...
def signal_within_n(check_func, df, n=3, reverse_check=None):
    # ✅09170846 ★n=3 的意思：近5根同時滿足碰軌與轉折（主帥 09/17 08:46 裁示保留 3）：訊號在【最近 3 根K棒】內出現過、且之後沒有出現反轉訊號，就仍算有效＝「訊號有效期」。★與「近5根碰軌」是兩件事，不可混用
    #   例：月K 前2根曾發出買進訊號、之後沒有賣出訊號 → 本根仍視為第一道通過。
    """✅ v05181836：近5根同時滿足碰軌與轉折 — 過去n根K棒內有訊號且無反轉，全市場通用
...
def scan_condition_w():
    ...
    if CONDW_FORCE_TEST:
        ...
        _tmsg = ('🧪【測試信．非進場訊號】\n'
                 ...
                 f'最後一根5分K：{_tb}\n'
                 f'時間：{now_str_f}\n\n'
                 '★本信由排程「強制測試」勾選框觸發；不勾選即為正式模式，不必改程式（09231655 更正）。')
    ...
    try:
        ...
        if df5 is not None and not df5.empty and _bar_too_old(df5, '條件W-5分K'):
            ...
            return
        _r5 = _condw_gate3(df5, CONDW_LOOKBACK, '5分K')   # ✅09030300 回到54（主帥：要敏感）
        if _r5:
            _res.append(_r5)
...
def load_cross_run_cache():
    ...
    try:
        ...
        cache_long_1mo.update(_wok); cache_mid_1wk_1d.update(_dok)
...
def save_cross_run_cache(prev_w=0, prev_d=0):
    ...
    try:
        import pickle
        if len(cache_long_1mo) <= prev_w and len(cache_mid_1wk_1d) <= prev_d:
            return
        today = datetime.now(pytz.timezone('Asia/Taipei')).strftime('%Y-%m-%d')
        with open(_kline_cache_path(), 'wb') as f:
            pickle.dump({'date': today, 'weekly': cache_long_1mo, 'daily': cache_mid_1wk_1d}, f)
        print(f'💾 [方案②] 已保存今日跨輪快取：月K {len(cache_long_1mo)} 檔、週K {len(cache_mid_1wk_1d)} 檔')
    except Exception as _e:
...
def check_holdings_health():
    ...
    _mk_order = ['台股', '美股', '虛擬幣', '外匯', '黃金']   # ✅09181118 補黃金，原本黃金持股示警不會出現在主旨
...
# ✅09180234【個股短線路線開關】主帥 09/18 01:22：「加一個『短線路線開關』，預設關閉，長中期路線照常。」
#   ★管的是【個股短線】（台股、美股、虛擬幣、外匯、黃金：日K OR 60分K → 30分K → 5分K OR 15分K）。
#   ★★與網頁版【期貨5分K】開關【無關】；★★★期貨與週選擇權（條件W、台指）不受本開關影響。
#   ★要做台股當沖或短線時改 True；主帥現階段以長期／中期投資為主，故預設 False。
STOCK_SHORT_ROUTE_ENABLED = False   # False＝只跑長期／中期即時路線；True＝同時跑短線路線


def _exit_stage1_ok(df1, is_long, label=''):
    """✅09190152【出場前提：第一道週期近N根至少碰過一次軌道】主帥 2026/09/19 01:52 指示。
    ・主帥原話（09/19 01:27 截圖）：「所有投資種類的週K，若已經超過連續5根K棒遠離上軌，
      就不用逃命了，因為已損失慘重！不准再頻繁寄發平倉出場通知信困擾我！」
    ・規則：多方要平倉 → 第一道週期【近 TOUCH_LOOKBACK_BARS 根】至少 1 根【最高價 ≥ 近上軌門檻】；
            空方要回補 → 至少 1 根【最低價 ≤ 近下軌門檻】（鏡像）。
    ・第一道週期：長期／中期＝週K；短線／期貨與週選＝^TWII 60分K 或 EWT 60分K。
    ・★都沒碰到 → 代表部位早已遠離出場區、損失已成事實，★★不再發平倉／回補通知。
    ・★★★資料不足或取得失敗 → 回傳 True（放行），★出場判斷不得因缺資料而被靜默擋掉。
    """
    try:
        if df1 is None or len(df1) < 20 or 'boll_top20' not in df1.columns:
            return True
        _n = min(TOUCH_LOOKBACK_BARS, len(df1))
        _bt = df1['boll_top20']; _bb = df1['boll_bot20']
        if is_long:
            _th = _gate_upper(_bt, _bb)
            _hit = any(float(df1['High'].iloc[-k]) >= float(_th.iloc[-k]) for k in range(1, _n + 1))
        else:
            _th = _gate_lower(_bt, _bb)
            _hit = any(float(df1['Low'].iloc[-k]) <= float(_th.iloc[-k]) for k in range(1, _n + 1))
        if not _hit:
            print(f"  🔕 出場前提未過{('（' + label + '）') if label else ''}：第一道近 {_n} 根未碰"
                  f"{'上' if is_long else '下'}軌 → 不發{'平倉' if is_long else '回補'}通知（主帥 09/19 01:52）")
        return _hit
    except Exception:
        return True
...
def _intraday_third_gate(df, g1l, g1s, g2l, g2s, is_long, is_short):
    ...
    try:
        ...
        # ✅09191108【第三道改用 stage_pass】主帥 09/19 09:43「乙的加強版」：三道同一套規則。
        #   ★一般進場：第一道 AND 第二道 AND 第三道(stage_pass)；★★條件E／eLeader 不再單獨使第三道成立。
        _s3_long  = stage_pass(df, True,  entry=True)
        _s3_short = stage_pass(df, False, entry=True)
        buy_n = bool(g1l and g2l and _s3_long and r_now > BUY_RSI_MIN)
        #   ★★★【為什麼是 AND】主帥 09/18 02:34、09:37：早期第一道＝月K、第二道＝週K，由大至小，
        #     ★那時寫 or 是對的（月K 或 週K 任一達標即可）；
        #     ★★現在第一道本身已經是【月K OR 週K】（短線是【日K OR ^TWII 60分K OR EWT 60分K】），
        #     ★★★『二擇一』已經在第一道內部完成，所以第一道與第二道之間必須是 AND，條件D 才有前兩道保護。
        buy_d = bool(g1l and g2l and check_condD_long(df))        # ✅09180234 條件D：第一道 AND 第二道
        sell_n = bool(g1s and g2s and _s3_short and r_now < SHORT_RSI_MAX)
        sell_d = bool(g1s and g2s and check_condD_short(df))      # ✅09180234 條件D 空方鏡像
        buy, sell = (buy_n or buy_d), (sell_n or sell_d)
        if buy and sell:   # ✅09191108 主帥 09/19 10:15：多空同時成立【兩邊都不發】，寧可空等
            print('  ⚖️ 多空同時成立 → 依主帥 09/19 10:15 裁示，兩邊都不發進場訊號')
            buy = sell = False
        # ✅09191108 出場改用 stage_pass(entry=False)：碰軌方向相反＋OSC 前置＋以最低／最高價判轉折；★不看實體
        close = bool(is_long and stage_pass(df, True, entry=False))
        cover = bool(is_short and stage_pass(df, False, entry=False))
...
def _osc_pre_ok(mh, is_long):
    """✅09191108【OSC 前置】主帥 09/19 10:17、11:08：
    ・做多進場：前 2 根→前 1 根 OSC【下降】（主帥稱「綠柱」）；做空進場鏡像【上升】。
    ・★主帥 11:08 定義：「我只看【升降】，不論是綠色還是紅色的柱狀體，
      我只看【近5根的其中某1根】圓柱有沒有比前1根圓柱高或低」→ ★★只比升降，不看正負。
    """
    try:
        _m2 = float(mh.iloc[-3]); _m1 = float(mh.iloc[-2])
        return (_m1 < _m2) if is_long else (_m1 > _m2)
    except Exception:
        return False


def _turn_ok(df, is_long):
    """✅09191108【轉折】前 1 根→當根：OSC 轉向 ＋ 以【最高／最低價】比前 1 根收盤（主帥 09/19 10:47）。
    ・★主帥原話：月K 若用收盤價，觸發日 9/15 要拖到 9/30 收盤才算數 → ★★當根一律用最高／最低價。
    ・做多：OSC 轉升 AND 當根最高價 > 前 1 根收盤；做空鏡像（OSC 轉跌 AND 當根最低價 < 前 1 根收盤）。
    """
    try:
        _mh = df['macd_hist']
        _m1 = float(_mh.iloc[-2]); _m0 = float(_mh.iloc[-1])
        _c1 = float(df['Close'].iloc[-2])
        if is_long:
            return (_m0 > _m1) and (float(df['High'].iloc[-1]) > _c1)
        return (_m0 < _m1) and (float(df['Low'].iloc[-1]) < _c1)
    except Exception:
        return False


def _body_ok(df, is_long):
    """✅09191108【實體】只看【前 1 根】：做多要求前 1 根不得收黑（收盤 ≥ 開盤）；做空鏡像。
    ・★主帥 09/19 11:08：「當根絕不能用【收盤價】，會拖延到 9/30 才逃命」→ ★★當根不看實體。
    ・★★★只用於【進場】；出場不套（主帥：連跌 11 根還要等紅K才逃，出場沒有意義）。
    """
    try:
        _o1 = float(df['Open'].iloc[-2]); _c1 = float(df['Close'].iloc[-2])
        return (_c1 >= _o1) if is_long else (_c1 <= _o1)
    except Exception:
        return False


def _touch_ok(df, is_long, n=None):
    """✅09191108【碰軌】近 N 根（預設 TOUCH_LOOKBACK_BARS＝5）：
    做多看最低價碰下軌、做空看最高價碰上軌；門檻為通道制與百分比制中【較嚴】者。"""
    try:
        _n = TOUCH_LOOKBACK_BARS if n is None else n
        _bt = df['boll_top20'].iloc[-_n:]; _bb = df['boll_bot20'].iloc[-_n:]
        if is_long:
            return bool((df['Low'].iloc[-_n:] <= _gate_lower(_bt, _bb)).any())
        return bool((df['High'].iloc[-_n:] >= _gate_upper(_bt, _bb)).any())
    except Exception:
        return False


def stage_pass(df, is_long, entry=True):
    """✅09191108【三道統一判斷】主帥 2026/09/19 09:43「乙的加強版」、10:15～11:08 逐項定義。
    ★進場（entry=True）：碰軌(近5) AND OSC 前置 AND 轉折 AND 前1根實體。
    ★★出場（entry=False）：碰軌(近5，方向相反) AND OSC 前置(方向相反) AND 轉折(方向相反)；★不看實體。
    ★★★第一道／第二道／第三道【都用本函式】，三道之間為 AND（主帥 09/19 10:47 b、c）。
    ・本函式取代舊的「條件A/B/C＋近5根同時滿足碰軌與轉折」通道：★舊制允許 3 根前的舊訊號仍算數，
      導致美股 MMM 在月K 前1根轉黑、週K OSC 轉降時仍發出買進（主帥 09/19 09:23 三張截圖）。
    ・出場方向對照：做多平倉＝看上軌＋OSC 前置上升＋當根最低價跌破前1根收盤（A轉）；做空回補鏡像。
    """
    try:
        if df is None or len(df) < 25 or 'macd_hist' not in df.columns:
            return False
        _dir = is_long if entry else (not is_long)      # 出場時碰軌與轉折方向相反
        if not _touch_ok(df, _dir):
            return False
        if not _osc_pre_ok(df['macd_hist'], _dir):
            return False
...
def _gate_one(fn_pre, fn_e, fn_el, df, within):
    """✅09191108【單一週期的一道判斷．改寫】主帥 2026/09/19 09:43～11:08：
    ・★一律呼叫 stage_pass()：碰軌(近5) AND OSC 前置 AND 轉折(高／低價) AND 前1根實體。
    ・★★【移除舊制】原式為「(條件A/B/C，可套 3 根內有效) OR 條件E OR eLeader」，
      ★★★該寫法讓 3 根前的舊訊號、或條件E／eLeader 單獨成立即可通過 → 本輪廢除。
    ・參數 fn_e／fn_el／within 保留以相容呼叫端，★但不再影響判斷結果。
    """
    _is_long = (fn_pre is check_buy_precondition)
...
def _intraday_route_gates(dm, dw, dd, d60, d30):
    ...
    _S4 = ((_gate_one(*B, dd, True) or _gate_one(*B, d60, True), _gate_one(*S, dd, True) or _gate_one(*S, d60, True),
            _gate_one(*B, d30, False), _gate_one(*S, d30, False))
           if STOCK_SHORT_ROUTE_ENABLED else (False, False, False, False))   # ✅09180234 短線路線開關關閉時，短線四道一律不成立
...
def _intraday_gates(df_d, df_30):
    """✅09171036 第一道（日K，近5根同時滿足碰軌與轉折 n=3）與第二道（30分K）。回傳 (第一道多, 第一道空, 第二道多, 第二道空)。"""
    def _g(fn_pre, fn_e, fn_el, df, within):
        if df is None or len(df) < 25:
            return False
        try:
            a = signal_within_n(lambda d: fn_pre(d)[0], df, n=3) if within else bool(fn_pre(df)[0])   # n=3＝（舊制字樣已移除，見檔尾附錄）
        except Exception:
            a = False
        try: e = bool(fn_e(df))
...
def _intraday_market_gate(cat):
    """✅09180234【守門員：大盤先過，個股也要過】主帥 09/18 01:22「大盤要先過，個股也要過」、02:05「守門員不再管出場，只要管進場」。
    ・台股看加權指數 ^TWII、美股看道瓊 ^DJI（守門員原始定案）；虛擬幣、外匯、黃金無對應大盤 → 一律放行。
    ・條件D 進場用同一個標準：條件D 本來就要先過第一道與第二道（主帥 09/18 02:05），故不再與「大盤條件D」配對。
    ・回傳 (多方可進場, 空方可進場)；大盤分析失敗一律放行，★守門本身不得成為漏訊號的原因。
    """
    _idx = {'tw': ('^TWII', '台股'), 'us': ('^DJI', '道瓊')}.get(cat)
    if not _idx:
        return (True, True)
    if cat in _mkt_gate_cache:
        return _mkt_gate_cache[cat]
    try:
        _m = analyse_market_index(_idx[0], _idx[1])
        _r = (bool(_m.get('bull_abc')), bool(_m.get('bear_abc')))
    except Exception as _e:
        print(f'  ⚠️ 守門員（{_idx[1]}）分析失敗（{str(_e)[:40]}）→ 放行')
        _r = (True, True)
    _mkt_gate_cache[cat] = _r
    print(f"  🛡️ 守門員（{_idx[1]}）：多方{'✅' if _r[0] else '❌'}　空方{'✅' if _r[1] else '❌'}")
    return _r


def _touch_detail(df, is_long, n=None):
    """✅09180924【碰軌明細】回傳 (方向文字, 第幾根, 轉折文字)，供通知信寫清楚是碰上軌還是下軌、V轉或A轉。
    主帥 09/18 08:25：「所以近5根K棒是觸碰上軌還是下軌，沒有提示給我怎麼知道是V轉或A轉」。
    ・近 n 根（預設 TOUCH_LOOKBACK_BARS＝5）由新到舊找第一根碰軌者，回傳「第 x 根」（1＝當根）。
    ・轉折：多方看前1根→當根 RSI↑ 且 MACD柱↑（V轉）；空方看 RSI↓ 且 MACD柱↓（A轉）。
    """
    _n = TOUCH_LOOKBACK_BARS if n is None else n
    try:
        _bt = df['boll_top20']; _bb = df['boll_bot20']
        _th = _gate_lower(_bt, _bb) if is_long else _gate_upper(_bt, _bb)
        _px = df['Low'] if is_long else df['High']
        _hit = 0
        for _k in range(1, min(_n, len(df)) + 1):
            _ok = (float(_px.iloc[-_k]) <= float(_th.iloc[-_k])) if is_long else (float(_px.iloc[-_k]) >= float(_th.iloc[-_k]))
            if _ok:
                _hit = _k
                break
        _r0 = float(df['rsi14'].iloc[-2]); _r1 = float(df['rsi14'].iloc[-1])
        _m0 = float(df['macd_hist'].iloc[-2]); _m1 = float(df['macd_hist'].iloc[-1])
        if is_long:
            _turn = 'V轉成立（RSI↑ 且 MACD柱↑）' if (_r1 > _r0 and _m1 > _m0) else 'V轉未成立'
        else:
            _turn = 'A轉成立（RSI↓ 且 MACD柱↓）' if (_r1 < _r0 and _m1 < _m0) else 'A轉未成立'
        _band = '下軌' if is_long else '上軌'
        return (_band, _hit, _turn)
    except Exception:
        return ('下軌' if is_long else '上軌', 0, '轉折資料不足')


def _gate_mark(ok):
    """✅09180924 通知信不再出現 True／False（主帥 09/18 08:26：「看不懂，這封信當時的K棒，究竟是多方還是空方」）。"""
    return '✅' if ok else '✗'
...
def _intraday_body(tk, label, sig, name, df, gates, routes, is_long_sig, now_str, stage1_txt='', pos_long=None, pos_frames=None):
    ...
    _bt = float(df['boll_top20'].iloc[-1]); _bb = float(df['boll_bot20'].iloc[-1])
    _touch = (f"近 {TOUCH_LOOKBACK_BARS} 根中第 {_hit} 根碰{_band} → {_turn}" if _hit
              else (f"近 {TOUCH_LOOKBACK_BARS} 根未碰{_band}（條件D 追高／追低不套碰軌）" if sig in ('buyD', 'sellD')
                    else f"近 {TOUCH_LOOKBACK_BARS} 根未碰{_band}"))
    # ✅09190211【方向】是【部位方向】（平倉＝做多部位出場、回補＝做空部位回補），
    #   ★與【看哪一條軌】是兩件事：★★做多平倉看上軌(A轉)、★★★做空回補看下軌(V轉)（主帥 09/19 02:11）。
    _dir = '做多' if (pos_long if pos_long is not None else is_long_sig) else '做空'
    return (f"【{name}】{label}　{tk}\n"
            f"・方向：{_dir}\n"
...
def _intraday_emit(codes, real, key5, frames, long_set, short_set, title, cat=None, daily_once=False):
    ...
    _merge_buf = []; _sig_head = [None]   # ✅09180924 每日一封的合併緩衝
    for c in codes:
        tk = real[c]
        ...
        for label, df in (('5分K', df5), ('15分K', frames['d15'].get(tk))):
            ...
            if sig in ('buy', 'buyD', 'sell', 'sellD'):   # ✅09180234 守門員只管進場；平倉、回補（出場）不受限制
                ...
                if sig in ('buy', 'buyD') and not _mb:
                    print(f'  🛡️ {tk} {label} 個股多方觸發，但大盤守門員未過 → 不發信')
                    continue
                if sig in ('sell', 'sellD') and not _ms:
                    print(f'  🛡️ {tk} {label} 個股空方觸發，但大盤守門員未過 → 不發信')
                    continue
            ...
            #   原本 close 被當成多方而去找下軌，信上寫成「近5根第2根碰下軌」與出場語意相反（主帥 09/19 01:27 截圖）。
            _band_long = (sig in ('buy', 'buyD')) or (sig == 'cover')
    ...
    # ✅09180924【每日一封】本輪合併信：把同一時刻觸發的所有標的與週期寫進同一封寄出
    if _merge_buf:
        _icon, _first = (_sig_head[0] or ('⭐', '進場'))
        _subj = f"☁️【雲端】{_icon}【{title}．當日合併】{len(_merge_buf)} 檔進場訊號 - {_now}"
        _body = ("☁️【雲端】" + f"{_icon}【{title}．當日合併通知】{_icon}\n"
                 + f"本信合併當下觸發的 {len(_merge_buf)} 檔進場訊號；★同一標的同一方向當天只通知一次。\n"
                 + "★出場（平倉、回補）不受此限，仍即時單獨寄出。\n"
                 + "══════════════════════════════════════\n"
                 + "\n──────────────────────────────────────\n".join(_merge_buf))
        _ok2 = send_gmail(_subj, _body, urgent=True)
        print(f"  {'✅' if _ok2 else '❌'} 每日一封：合併寄出 {len(_merge_buf)} 檔")
...
def scan_stock_intraday_tw():
    ...
    try:
        ...
        _intraday_emit(codes, real, lambda c: c + '.TW',   # ✅09172001 逐檔判斷抽出為共用函式，台股行為不變
                       ...
                       set(str(x) for x in HOLDINGS_TW), set(str(x) for x in HOLDINGS_SHORT), '個股即時', cat='tw')
    except Exception as _e:
...
def scan_stock_intraday_global():
    ...
    try:
        ...
        _intraday_emit(codes, real, lambda c: c, frames, long_set - short_set, short_set, '即時', daily_once=True)   # ✅09180924 美股／虛擬幣／外匯／黃金：進場每標的每方向每天一封
    except Exception as _e:
...
def scan_futures_15mk(gates=None):   # ✅09170937 gates＝{標的: (第一道多, 第一道空, 第二道多, 第二道空)}；未提供時只檢查平倉回補
    ...
    ・回看根數：★碰軌丙案後【不再等比換算】，5分K 與 15分K 一律近 5 根（TOUCH_LOOKBACK_BARS）
      （舊制「5分K 54 根 → 15分K 18 根」等比換算已作廢，見檔尾附錄）
    ...
    try:
        ...
        for _tk in FUTURES_5MK_TARGETS:
            try:
                ...
                #   （沿革見檔尾【附錄．程式註解沿革存查】H-022）
                _bb_gate   = _gate_lower(_bt.iloc[-_n15:], _bb.iloc[-_n15:])
                _bt_gate   = _gate_upper(_bt.iloc[-_n15:], _bb.iloc[-_n15:])

                # 多方：近5根任一最低價觸及布林下軌帶（位階）AND 近5根碰下軌（附加）AND 前1根→當根 RSI↑ AND MACD柱↑   # ✅09181106 18→5（碰軌丙案）
                ...
                #     ★★現在第一道本身已經是【月K OR 週K】（短線是【日K OR ^TWII 60分K OR EWT 60分K】），
                #     ★★★『二擇一』已經在第一道內部完成，所以第一道與第二道之間必須是 AND，條件D 才有前兩道保護。
                _buy  = bool(_buy or (_g1l and _g2l and check_condD_long(_df)))   # ✅09180234 條件D 路徑：第一道 AND 第二道（不套碰軌）
                # 空方鏡像：近5根任一最高價觸及布林上軌帶 AND 前1根→當根 RSI↓ AND MACD柱↓ AND 近5根碰上軌   # ✅09181106 18→5
                ...
                #   主帥 09/17 02:55：「5分K和15分K都要改看【近5根有沒有碰到上下軌?】+【前1根和當根比較,macd和rsi有沒有V轉或A轉?】」
                # ✅09190152 取第一道（^TWII 60分K，取不到再退 EWT 60分K）供出場前提判斷
                _df1st_15 = None
                for _sym in (_tk, 'EWT'):
                    try:
                        _d1 = yf.download(_sym, period='3mo', interval='60m', progress=False)
                        if _d1 is not None and len(_d1) >= 30:
                            _df1st_15 = calc_indicators(_normalize_df(_d1))
                            if _df1st_15 is not None:
                                break
                    except Exception:
                        continue
...
def main_task():
    ...
    # 🔥 宣告 global 確保全域共用
    global cache_long_1mo, cache_mid_1wk_1d, buy_signals, sell_signals, delist_signals, _futures_is_holding, _futures_is_short  # ✅ 07031936 加入期貧持倬全域
    global _holdings_sent   # ✅ (07130626) 持股每日健檢
    global _prescreened_ind   # ✅09180234【缺陷修正】原未宣告 global → 下方 `_prescreened_ind = {}` 變成區域變數，
                              #   而 scan_stock 寫入的是全域 → 上傳時取到空字典，雲端紀錄長期顯示「指標 0 支」（主帥 09/18 02:34 提供 09/16 log 佐證）
    ...
    if now_hour == WEEKLY_REFRESH_HOUR and len(cache_long_1mo) > 0:
        print(f"♻️ [{now_str}] 偵測到新的一天, 正在清除快取資料以獲取最新K線...")
        cache_long_1mo.clear()
        cache_mid_1wk_1d.clear()

    # ✅ 方案②(07061319) 記憶體快取為空(新程序或剛清空)時，載入今日跨輪快取以免重抓
    if USE_CROSS_RUN_CACHE and len(cache_long_1mo) == 0 and len(cache_mid_1wk_1d) == 0:
        load_cross_run_cache()
    _xrun_prev_w = len(cache_long_1mo); _xrun_prev_d = len(cache_mid_1wk_1d)   # 記錄載入後基準，供結束時判斷是否成長
    ...
    print(f"\n{'='*55}")
    print(f"  股票買進賣出建議系統啟動（{"期貨掃描模式(5分K+15分K)" if TEST_MODE in (chr(39)+"5mk"+chr(39), chr(39)+"futures"+chr(39)) else f'{SCAN_MODE}模式'}）  v{SCRIPT_VERSION}")
    print(f"  掃描時間：{now_str}")
    print(f"  本次掃描市場：{', '.join(active_markets)}")
    print(f"{'='*55}")

    # 掃描前預先抓取全額交割清單（快取後不重複抓）
    _skip_cash = (TEST_MODE in ('5mk', 'futures')) or ('TW' not in active_markets and 'CRYPTO' not in active_markets)
            ...
            try:
                ...
                if result:
                    if result[0] == 'BUY':
                        ...
                        # ✅09180937【守門員只管進場】主帥 09/18 02:05：「守門員不再管出場，只要管【進場】就好。」
                        #   ★原本持股賣出（出場）也要大盤空頭才發信 → 大盤仍多頭時，持股碰上軌 A轉該逃命卻收不到信。
                        sell_signals.append(('台股', code, *result[1:]))
                    elif result[0] == 'SHORT':
                        # ✅09180937【補上台股做空進場】主帥 09/16 17:16：「策略如果只有多方，則 AI 必須鏡像產出空方」；
                        #   ★原本 scan_stock 算得出 'SHORT'，main_task 卻沒有接收 → 訊號算完就消失。
                        ...
                        if _mkt_match_s:
                            sell_signals.append(('台股做空', code, *result[1:]))
                        else:
                            print(f'  🛡️ {code} 個股空頭觸發但大盤守門員未過，不發通知')
                    elif result[0] == 'COVER':
        ...
        for ticker in US_STOCKS:
            ...
            if result:
                if result[0] == 'BUY':
                    ...
                    # ✅09180937 守門員只管進場：美股持股出場同樣不受大盤限制（主帥 09/18 02:05）
                    sell_signals.append(('美股', ticker, *result[1:]))
                elif result[0] == 'SHORT':
                    # ✅09180937 補上美股做空進場（鏡像原則，主帥 09/16 17:16）
                    ...
                    if _mkt_match_us_s:
                        sell_signals.append(('美股做空', ticker, *result[1:]))
                    else:
                        print(f'  🛡️ {ticker} 美股個股空頭觸發但大盤守門員未過，不發通知')
                elif result[0] == 'COVER':
    ...
    if 'FUTURES' in active_markets and TEST_MODE in ('5mk', 'futures'):
        ...
        for ticker in FUTURES_5MK_TARGETS:
            try:
                ...
                # 第三道：5分K OR 15分K 條件A/B + RSI門檻（近N根一律 5，BUY_LOOKBACK_5MK=5）   # ✅09181106 54→5
                # ══════════════════════════════════════════════
                ...
                # 【正確先大後小架構】：
                #   第一道 → 日K（近5根同時滿足碰軌與轉折=3個交易日）← 大週期，看整體位階
                ...
                # ✅09181118 不在此 continue：條件D 前提為【第一道 AND 第二道】，且有持倉時必須照常檢查平倉回補
                ...
                # ── 第二道：EWT 30分K（美股時段交易，涵蓋台灣夜盤）──────────   # ✅09181106 標題更正：實作本來就是 EWT 30分K，不是日K eLeader
                #     → ★★這是【做多】的 eLeader，★但 continue 會跳過【整支標的】。
                #   ★★★後果：大跌時做多 eLeader 幾乎必然不過 → 整支跳掉 →
                #     ★下面的 _fut_short 根本執行不到 → ★★做空在結構上永遠不可能觸發。
                ...
                # ── 第三道：5分K OR 15分K 條件A/B（近5根）────────────   # ✅09181106
                ...
                # ── 條件A：近5根任一最低價 <= 近下軌門檻（通道制取較嚴）AND 前1根→當根 RSI↑ AND MACD柱↑   # ✅09181106
                # ✅08301905 容忍度改綁通道寬度；原式：bb5.iloc[-n5:] * BUY_BOLL_TOLERANCE
                _fut_cond_A = (l5.iloc[-n5:] <= _gate_lower(bt5.iloc[-n5:], bb5.iloc[-n5:])).any() and rsi_rising and macd_rising
                # ── 條件B：近5根最低均<布林中軌 AND 最高均<布林上軌 AND 前N根MACD縮 AND 當根MACD放大   # ✅09181106
                _fut_low_mid  = (l5.iloc[-n5:] < bm5.iloc[-n5:]).all()
                _fut_high_top = (h5.iloc[-n5:] < bt5.iloc[-n5:]).all()
                ...
                #   ★★★check_condE_short 與 check_short_eleader 兩函式【早已存在】
                #     （第一道 EWT 於 5299~5300 行就在用），★只是第三道沒接上。
                _fut_cond_E_short = check_condE_short(df5) if df5 is not None else False
                _fut_cond_F_short = check_short_eleader(df5) is not None if df5 is not None else False
                _fut_cond_E_long = check_condE_long(df5) if df5 is not None else False
                _fut_cond_F_long = check_buy_eleader(df5) is not None if df5 is not None else False
                ...
                #   ★09/14 08:21「條件D 只會發生在第三道，絕對不可發生在第一道 or 第二道」＝條件D 不得當作第一、二道的通過條件，★不是可以略過前兩道（09170937 誤解為 or，本輪更正）
                #   ★主帥 09/18 02:34：「既然第一道已改成【月K or 週K】，那麼【第一道 and 第二道 and 條件D】也是正確的」；條件D 仍不套近5根碰軌（主帥 09/17 08:34 例外）
                #   ★★★【為什麼是 AND】主帥 09/18 02:34、09:37：早期第一道＝月K、第二道＝週K，由大至小，
                #     ★那時寫 or 是對的（月K 或 週K 任一達標即可）；
                #     ★★現在第一道本身已經是【月K OR 週K】（短線是【日K OR ^TWII 60分K OR EWT 60分K】），
                #     ★★★『二擇一』已經在第一道內部完成，所以第一道與第二道之間必須是 AND，條件D 才有前兩道保護。
                _fut_buy_D = bool(_1st_long and _buy_2nd and _fut_cond_D_long)
                now_str_f = datetime.now(pytz.timezone('Asia/Taipei')).strftime('%Y/%m/%d %H:%M')
                # ── 日K位階參考（印出但不擋住掃描）──────────────
                try:
                ...
                # 第二道：日K eLeader 25條件全部反向
                # 第三道：5分K OR 15分K 條件A/B 反向（近5根）+ RSI↓ AND MACD柱↓   # ✅09181106
                # ══════════════════════════════════════════════
                ...
                _fut_short_B   = _fut_high_mid and _fut_low_bot and _fut_macd_exp and macd_falling
...
# ============================================================
    ...
    #   （沿革見檔尾【附錄．程式註解沿革存查】H-031）
    if TEST_MODE in ('5mk', 'futures', 'condW'):
...
# =====================
    ...
    if buy_signals:
        ...
        for s in buy_signals:
            ...
            # ✅09180937【每日一封】主帥 09/18 09:07、09:24：美股、外匯、虛擬幣、基金、黃金的【進場】通知，
            #   ★同一標的當天只寄一次（額度 1）；★★台股與期貨維持原額度（日盤 3、夜盤 2）。
            #   ★★★出場（賣出、回補）不受此限，仍照原額度即時寄出（主帥：出場是逃命）。
            _once_mkts = ('美股', '虛擬幣', '外匯', '黃金', '債券基金', '基金', '美股做空')
            _mx = 1 if market in _once_mkts else _sess_max
...
# ══════════════════════════════════════════════════════════════
# 【附錄．程式註解沿革存查】
#   ★本附錄只存放【已被取代的舊註解與沿革】，★★不得據以操作；現行規格以主文與首頁文件為準。
#   ★★★搬移原因：主帥 09/17 23:07「很多已經取消或【從新從後原則】改以新規定為主，結果在主文也充斥著舊規定！！！」
# ══════════════════════════════════════════════════════════════
# ── 自我舉證表(09021330) 與其後各輪追加（結論①~⑳、推定1~6） ──
...
# ── 檔頭說明（09030300 版原文：三版一致由來、5mk 名稱、週一12:50~週三11:30、只適用週K）──
# ============================================================
...
# ── 第三批：主文沿革敘述總清理（09181118，共 34 段）──
# ── H-001（原位置：# ══════════════════════════════════════…）──
#   ★主帥原話：「你只是在對話視窗建議我設定成【TEST_MODE = 'futures'】是沒用的，我又不是AI可以1秒快速查找關鍵字！

# ── H-002（原位置：# ── 【２-1】買進策略：截圖3、4條件（條件A + 條件B）───────…）──
#   ★原值：月K/週K 3、日K 3、5分K 54（03/25 文件依「K棒等比換算」推得，已作廢）

# ── H-003（原位置：# ── ✅09022155【期貨未平倉·收盤前提醒信】主帥 2026/09/0…）──
#   ★主帥原話：「我期貨進場，極少放任到收盤，幾乎9成次數會在13:20前平倉！

# ── H-004（原位置：# ✅08301755【15分K 容忍度改綁通道寬度】★本次做空誤訊號的根因修正…）──
...
# ── H-007（原位置：# ✅09151754【主帥 2026/09/15 07:15 核准甲＋乙】★★…）──
    #   ★【原本的錯】虛擬幣被綁在 is_us_time（21:30~04:00）裡 →

# ── H-008（原位置：# ══ ✅09021415【期貨時窗＝條件W 時窗，單一真實來源】主帥 09/…）──
    #   ★★★這裡原本有【三段】各自為政的判斷（週一~週三窗／週選夜盤窗／日盤補齊），
    #     ★★★而 _condw_current_window() 又是第四份抄本 —— 共四份、四種寫法。

# ── H-009（原位置：# ✅08250517【★★跨棒差分．重大修正】…）──
        #   ★★★所以我上一輪說「迴圈取樣只是補強、缺它不影響核心」是【錯的】，

# ── H-010（原位置：# ======================================…）──
#   ・查證：網頁版 saveToCloud 早已把持股清單寫入 artifacts/{專案}/users/{safeId}/data/stocks；雲端版原本只讀 watchlist。

# ── H-011（原位置：# ✅09131152【缺陷G】發信失敗必須留痕。★原本只 print 一行就 …）──
        # ✅09131152【缺陷G】發信失敗必須留痕。★原本只 print 一行就 return False，

# ── H-012（原位置：# ── 條件A（截圖3、4）─────────────────────────…）──
        # ✅08301905 容忍度改綁通道寬度（取兩制較嚴者）；原式：bb.iloc[-n:] * BUY_BOLL_TOLERANCE

# ── H-013（原位置：# ── 條件D（上軌做多：大盤強多頭時，K棒在上軌附近進場）─────────…）──
        # ✅09170937【條件D 移出第一道】原本在此以週K 計算條件D，違反主帥規定；改由 check_condD_long() 於第三道判定。

# ── H-014（原位置：# ✅09141319【主帥 09/14 08:21 明示】★「條件D，★★只會…）──
        #   ★原式 `return _abc or cond_D` 讓條件D【能單獨讓第一道通過】→ 違反本則。

# ── H-015（原位置：# ── 第三道：5分K OR 15分K（✅09180937 主帥 09/18 …）──
        #   ★原本只有 5分K，且條件寫成「RSI↑ AND MACD↑ AND 5分K條件A/B」→ 家人親友房客沒有 15分K 可判斷 A轉／V轉。

# ── H-016（原位置：# ✅08092144【主帥指示關閉】收盤確認信…）──
        #   主帥原話：「我的設計理念是【收到通知信就3分鐘內迅速進場下單】。

# ── H-017（原位置：# ✅08061155【主帥指定】上限由50支改為 LIMIT_UP_MAX_S…）──
        #   預篩清單原本沒有市值排序，直接取前50支等於隨機取樣；

# ── H-018（原位置：# 窗一延續：週三(2)11:30止（窗ID用前一日週二）…）──
...
# ── H-020（原位置：# ✅08250451【修正·與 08102047 同型】★台指期K棒累積移到【…）──
    #   ★真實事故：08102047 已為 futures-scan 修過完全一樣的問題，

# ── H-021（原位置：# ✅09022155【★★★回正：多空【共用】同一組配額】…）──
...
# ── H-022（原位置：# ✅08301755【15分K 容忍度改綁通道寬度】多空兩側同步（ＡＫ１８）…）──
                # ✅09170232 P0③ 當根門檻已移除，以下兩行停用（保留原式供對照）

# ── H-023（原位置：# 空方鏡像：近5根任一最高價觸及布林上軌帶 AND 前1根→當根 RSI↓ A…）──
                #   ✅08301755【補上空方RSI門檻】SHORT_RSI_MAX 原本【宣告了但全檔零使用】，

# ── H-024（原位置：# ✅08102047【修正·問題A】台指期K棒累積移到【所有關卡之前】無條件執…）──
        #   真實事故：08091843 起把它放在「下載df5之後、陳舊判定之前」，

# ── H-025（原位置：# ✅09170846【三道丙案．先大後小】第一道＝日K；第二道＝EWT 30分…）──
                #   原式：第一道＝日K AND EWT 30分K，第二道＝EWT 30分K eLeader（註解誤寫日K）→ 大、中、中、小，第二道重複用 30分K

# ── H-026（原位置：# ── 第二道：EWT 30分K（美股時段交易，涵蓋台灣夜盤）────────…）──
                # ✅09022055【★★★修正：做空原本被做多的第二道 continue 綁架】
                #   ★原式：`if check_buy_eleader(df5_d) is None: continue`

# ── H-027（原位置：# ✅ v06160503修復：near_lower/near_upper 原定…）──
                # ✅09170658 碰軌附加條件改為近 TOUCH_LOOKBACK_BARS 根（原式只看當根收盤價：close <= _gate_lower(boll_top, boll_bot)）

# ── H-028（原位置：# ✅09022055 第一道、第二道改為【顯式帶入】。…）──
                #   ★原式靠上方的 continue 隱含保證，★★拆開多空後 continue 不再等價，

# ── H-029（原位置：# ✅09140742【多空鏡像修復】★原式只有 (_fut_short_A o…）──
                # ✅09140742【多空鏡像修復】★原式只有 (_fut_short_A or _fut_short_B) 兩個條件，

# ── H-030（原位置：# ✅ 08032126【重大補漏】做空進場原本【完全沒有】週選擇權推薦，…）──
                        # ✅ 08032126【重大補漏】做空進場原本【完全沒有】週選擇權推薦，

# ── H-031（原位置：# ✅08100036【⚪F′】補掃「全市場掃描範圍外」的觀察清單標的…）──
    #   真實事故：期貨5分K模式每5分鐘跑一次，原本會連帶每5分鐘讀一次 Firebase

# ── H-032（原位置：# ✅08101455【修正】自報區塊改到【所有通知處理完之後】才印。…）──
    #   真實事故：08100100 版把它掛在「掃描完成」那行後面，但通知處理（時段判定）

# ── H-033（原位置：# ══ ✅09021415【★★★期貨時窗與條件W 完全綁定】主帥 2026/…）──
        #   ★主帥原話：「條件W的週選擇權，既然都延長到11:30，期貨也要改成到11:30。

# ── H-034（原位置：# ══ ✅09021415【台股盤中極端異動 獨立時窗】★★★不得被期貨時窗綁…）──
        #     check_tw_intraday_extreme()（F-12）原本【寄生在期貨5mk 分支裡】。

# ── 第四批：根數、條件D 前提、夜盤門檻等主文用語更正（09181118）──
#     ★★★正確語意是：★5分K 用 54 根、★15分K 用 18 根（★兩個獨立數字）。
#   → ★依此原則，★★選【N 較大】者：5分K 54 根、15分K 18 根。
